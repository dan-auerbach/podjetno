...
import { getStories, getStoryById, getSponsors, addUtmParams, formatDate } from '@/lib/data';
...
export default async function EntrepreneurPage({ params }: PageProps) {
  ...
  const portalName = PORTAL_NAMES[story.portal] || story.portal;

  // Generic highlights
  const highlights = [
    'Rast podjetja in ključne odločitve',
    'Vodenje ekipe in kultura',
    'Lekcije in napake na poti',
  ];

  return (
    <>
      <article className="py-12 md:py-20 pb-32">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <header className="mb-8">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500 mb-4">
              <span className="font-medium text-slate-700">{portalName}</span>
              <span>•</span>
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm text-slate-600 hidden sm:block">Želiš celotno zgodbo?</p>
              <p className="text-sm font-semibold text-slate-900 truncate">{story.companyName}</p>
            </div>
            <a
              href={story.url}
              target="_blank"
              rel="noopener noreferrer"
              className="flex-shrink-0 inline-flex items-center justify-center px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition-colors gap-2 text-sm"
            >
              Preberi intervju
              <svg
                className="w-4 h-4"
